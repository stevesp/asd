...
public class ShellSort {
	...
	private static Integer ordinaSingolaCatena(Integer A[], Integer i,
			Integer inc) {
		...
		while (k < A.length) {
			...
			while (precedente >= i && !inserito) {
				...
				if (x < A[precedente]) {
					...
					inserito = true; // corrente rappresenta la posizione dove
										// inserire l'elemento
			}
			A[corrente] = x;
			k = k + inc;
	...
	public static Integer sort(Integer A[]) {
		...
		while (inc >= 1) {
			for (Integer i = new Integer(0); i < inc; i++)
				numeroConfronti = numeroConfronti
						+ ordinaSingolaCatena(A, i, inc);
			inc = inc / 2;
		}
