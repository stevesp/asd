...
public class AlberoVP implements Albero, Iterable {
	...
	public Nodo primoFiglio(Nodo v) throws EccezioneNodoInvalido,RuntimeException {
		if (checkNode(v))
			throw new EccezioneNodoInvalido();
	...
	public void insSottoAlbero(Nodo fratello, Albero a) throws EccezioneNodoInvalido{
		...
		tmpNodi[indice+1]=(NodoVP) ((AlberoVP)a).radice(); // innesto l'albero
		tmpNodi[indice+1].albero=this;
		
		tmpNodi[indice+1].indice=indice+1; //assegno un nuovo indice alla radice di a
		tmpPadri[indice+1]= (NodoVP) padre(fratello); // assegno il padre
	...
	public boolean fineFratelli(Nodo v) throws EccezioneNodoInvalido{
		if(checkNode(v)) throw new EccezioneNodoInvalido("nodo invalido");
