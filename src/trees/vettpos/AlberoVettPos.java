...
public class AlberoVettPos implements Albero, Iterable {
	...
	public Nodo primoFiglio(Nodo v) throws EccezioneNodoInvalido, EccezioneNodoInesistente{
		if(checkNode(v))
			throw new EccezioneNodoInvalido();
		//(d(p-1) + i+1)
	...
	public boolean foglia(Nodo v) throws EccezioneNodoInvalido{
		// verifico se ha figli (se la posizione del primo figlio restituisce un nodo)
		try {
			primoFiglio(v);
			return false;
	...
	//Shif a destra del sottoalbero radicato nel nodo in posizione i (la radice è già stata spostata
	...
	////innesta "a"  a partire dal nodo padre di this (la radice di a è già innestata in padre)
	private void innesta(Nodo padre, Albero a, Nodo aPadre){
		if(!a.foglia(aPadre)){
			Nodo u=a.primoFiglio(aPadre);
			int posPrimoFiglio = d * ((((NodoVPos) padre).indice) - 1)+ 1+ 1; //pos in this
			// inserisco u in this
			...
			while(!a.fineFratelli(u)){
				u=a.succFratello(u);
				int posSuccFiglio = d * ((((NodoVPos) padre).indice) - 1)+ j+ 1; //pos in this
				// inserisco u in this
	...
	public void insprimoSottoAlbero(Nodo padre, Albero a) throws EccezioneNodoInvalido	{	
		if (checkNode(padre))
			throw new EccezioneNodoInvalido();
		if(a.alberoVuoto())
		...
		//profondità a cui finirebbe l'ultimo livello di "a" una volta immesso in this
		int maxTemp=profCorrente+maxPa+1; //profondità massima a cui finisce l'ultimo livello di a una volta inserito in this

		// Alloco nuova memoria se necessario
		if(maxTemp>maxP){
			// bisogna estendere P
			int newDim=P.length;			
			for(int i=maxP+1;i<=maxTemp;i++) // alloca i necessari livelli
				newDim+=Math.pow(d,i);
		...
		P[posPrimoFiglio]=(NodoVPos)(((NodoVPos)((AlberoVettPos)a).radice()).clone());
		P[posPrimoFiglio].indice=posPrimoFiglio;
		P[posPrimoFiglio].arita=1;
