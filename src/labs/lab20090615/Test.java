...
public class Test {
	...
	public static void inizializza2(AOLista list) {
		...
		list.insert(new String("3"),
				list.pred(list.pred(list.pred(list.firstList())))); // => 3
	}
	...
	public static void stampaSD(AOLista list) {
		try {
			for (Posizione p = list.firstList(); !list.endList(p); p = list
					.succ(p))
	...
	public static void stampaDS(AOLista list) {
		try {
			// for(Posizione p = list.pred(list.firstList()); p !=
			// list.firstList(); p = list.pred(p))
			// System.out.println(list.readList(p));
			for (Posizione p = list.pred(list.pred(list.firstList())); p != list
					.firstList(); p = list.pred(p))
				System.out.println(list.readList(p));
	...
	public static boolean palindromo(AOLista list) {
		try {
			Posizione p1 = list.firstList(), p2 = list.pred(list.pred(list
					.firstList()));
			int size = size(list);
