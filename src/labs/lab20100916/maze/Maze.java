...
public class Maze {
	...
	private boolean puntoUscita(Posizione p) {
		return ((p.row == 0) || (p.column == 0) || (p.row == maze.length - 1) || (p.column == maze[p.row].length - 1)
	...
	public boolean valido(Posizione p) {
		if (p.row == maze.length || p.column == maze[p.row].length || p.row < 0
				|| p.column < 0)
			throw new EccezionePosizioneNonValida(
					"Posizione fuori dal labirinto");
	...
	public static void main(String[] args) {
		...
		if (lab.attraversa(iniziale))
			...
			System.out.println("Partendo dalla posizione " + iniziale
					+ " il labirinto non ha soluzioni");
	}
