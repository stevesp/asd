...
public class Test {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 15; i++) {
			...
			try {
				unrunnable.insert(processi[i],
						new Priority(processi[i].getPriority()));
		...
		int stato = 1;
		System.out
				.println("*** Sposto i processi nelle code unrunnable/stopped ***");
		while (runnable.size() != 0) {
			...
			if (stato == 0) {
				System.out.println("Sposto il processo " + tmp
						+ " in UNRUNNABLE");
